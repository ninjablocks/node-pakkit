...
var debug = require('debug')('pakkit');

var CORE_TYPES = ['uint8', 'uint16le', 'uint32le', 'buffer', 'string'];

//TODO: Add all the missing core types (monkey-patch concentrate+dissolve or fork?)
  ...
  export: function(packets, types) {
    ...
    Object.keys(packets).forEach(function(name) {
      ...
      Object.keys(inPacket).forEach(function(attName) {
        ...
        //debug('Checking attribute ' + attName);
      ...
      packet.write = function(values) {
        ...
        debug('-- Building', packet.name, ' with ', values);
        ...
        packet.attributes.forEach(function(attr) {
          ...
          debug('----- Set', value, 'on', attr, attr.name);
  ...
  Object.keys(types).forEach(function(name) {
      ...
      type.write = function(builder, value, attribute) {
        ...
        if (attribute.mask) {
          if (typeof value == 'object') {
            ...
            debug('Created mask for ', attribute.name, mask, value);
        ...
        if (attribute.enum) {
          switch(typeof value) {
            case 'string':
              value = attribute.enum[value];
              break;
            case 'number':
              value = attribute.enum.get(value);
              break;
          }
          value = value.value;
        }
    ...
    if (!type.read) {
      type.read = function(parser, attribute) {
        ...
        /*parser.tap(function() {
          debug('Read attribute', attribute.name, this.vars[attribute.name]);
        });*/
